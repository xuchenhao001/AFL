...
axes.bar([(p - width * 2) for p in range(len(x))], height=fed_async, width=width, label="AFL", hatch='x')
axes.bar([p - width for p in range(len(x))], height=fed_avg, width=width, label="FedAVG", hatch='o')
axes.bar(range(len(x)), height=fed_localA, width=width, label="APFL", hatch='+')
axes.bar([p + width for p in range(len(x))], height=fed_sync, width=width, label="SFL", hatch='*')
